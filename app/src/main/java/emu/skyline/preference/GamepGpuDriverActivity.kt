 ...
 */
@AndroidEntryPoint
class GamepGpuDriverActivity : AppCompatActivity() {
        ...
        if (it.resultCode == RESULT_OK) {
            it.data?.data?.let { uri ->
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    val result = GpuDriverHelper.installDriver(this@GamepGpuDriverActivity, fileStream)
                    runOnUiThread {
    ...
    private fun populateAdapter() {
        ...
        //var parentTitle = applicationContext.
        ...
        items.add(GpuDriverViewItem(GpuDriverHelper.getSystemDriverMetadata(this)) {
            preferenceSettings.gamepGpuDriver = PreferenceSettings.SYSTEM_GPU_DRIVER
        })

        if (preferenceSettings.gamepGpuDriver == PreferenceSettings.SYSTEM_GPU_DRIVER) {
        ...
        GpuDriverHelper.getInstalledDrivers(this).onEachIndexed { index, (file, metadata) ->
            items.add(GpuDriverViewItem(metadata).apply {
                onDelete = { position, wasChecked ->
                    ...
                    Snackbar.make(binding.root, "${metadata.label} deleted", Snackbar.LENGTH_LONG).setAction(R.string.undo) {
                        this@GamepGpuDriverActivity.adapter.run {
                            ...
                            if (wasChecked) {
                                ...
                                preferenceSettings.gamepGpuDriver = metadata.label
                            }
                        }
            ...
            if (preferenceSettings.gamepGpuDriver == metadata.label) {
